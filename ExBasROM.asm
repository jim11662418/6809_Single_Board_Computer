...
RTS_LOW   EQU  $95
          ORG  0
ENDFLG    DS   1              ;STOP/END FLAG: POSITIVE=STOP, NEG=END
CHARAC    DS   1              ;TERMINATOR FLAG 1
ENDCHR    DS   1              ;TERMINATOR FLAG 2
TMPLOC    DS   1              ;SCRATCH VARIABLE
IFCTR     DS   1              ;IF COUNTER - HOW MANY IF STATEMENTS IN A LINE
DIMFLG    DS   1              ;ARRAY FLAG 0=EVALUATE, 1=DIMENSIONING
VALTYP    DS   1              ;TYPE FLAG: 0=NUMERIC, $FF=STRING
GARBFL    DS   1              ;STRING SPACE HOUSEKEEPING FLAG
ARYDIS    DS   1              ;DISABLE ARRAY SEARCH: 00=ALLOW SEARCH
INPFLG    DS   1              ;INPUT FLAG: READ=0, INPUT<>0
RELFLG    DS   1              ;RELATIONAL OPERATOR FLAG
TEMPPT    DS   2              ;TEMPORARY STRING STACK POINTER
LASTPT    DS   2              ;ADDR OF LAST USED STRING STACK ADDRESS
TEMPTR    DS   2              ;TEMPORARY POINTER
TMPTR1    DS   2              ;TEMPORARY DESCRIPTOR STORAGE (STACK SEARCH)
FPA2      DS   4              ;FLOATING POINT ACCUMULATOR #2 MANTISSA
BOTSTK    DS   2              ;BOTTOM OF STACK AT LAST CHECK
TXTTAB    DS   2              ;BEGINNING OF BASIC PROGRAM
VARTAB    DS   2              ;START OF VARIABLES
ARYTAB    DS   2              ;START OF ARRAYS
ARYEND    DS   2              ;END OF ARRAYS (+1)
FRETOP    DS   2              ;START OF STRING STORAGE (TOP OF FREE RAM)
STRTAB    DS   2              ;START OF STRING VARIABLES
FRESPC    DS   2              ;UTILITY STRING POINTER
MEMSIZ    DS   2              ;TOP OF STRING SPACE
OLDTXT    DS   2              ;SAVED LINE NUMBER DURING A "STOP"
BINVAL    DS   2              ;BINARY VALUE OF A CONVERTED LINE NUMBER
OLDPTR    DS   2              ;SAVED INPUT PTR DURING A "STOP"
TINPTR    DS   2              ;TEMPORARY INPUT POINTER STORAGE
DATTXT    DS   2              ;'DATA' STATEMENT LINE NUMBER POINTER
DATPTR    DS   2              ;'DATA' STATEMENT ADDRESS POINTER
DATTMP    DS   2              ;DATA POINTER FOR 'INPUT' & 'READ'
VARNAM    DS   2              ;TEMP STORAGE FOR A VARIABLE NAME
VARPTR    DS   2              ;POINTER TO A VARIABLE DESCRIPTOR
VARDES    DS   2              ;TEMP POINTER TO A VARIABLE DESCRIPTOR
RELPTR    DS   2              ;POINTER TO RELATIONAL OPERATOR PROCESSING ROUTINE
TRELFL    DS   1              ;TEMPORARY RELATIONAL OPERATOR FLAG BYTE
...
FP0EXP    DS   1              ;FLOATING POINT ACCUMULATOR #0 EXPONENT
FPA0      DS   4              ;FLOATING POINT ACCUMULATOR #0 MANTISSA
FP0SGN    DS   1              ;FLOATING POINT ACCUMULATOR #0 SIGN
COEFCT    DS   1              ;POLYNOMIAL COEFFICIENT COUNTER
STRDES    DS   5              ;TEMPORARY STRING DESCRIPTOR
FPCARY    DS   1              ;FLOATING POINT CARRY BYTE
; FLOATING POINT ACCUMULATOR #1
FP1EXP    DS   1              ;FLOATING POINT ACCUMULATOR #1 EXPONENT
FPA1      DS   4              ;FLOATING POINT ACCUMULATOR #1 MANTISSA
FP1SGN    DS   1              ;FLOATING POINT ACCUMULATOR #1 SIGN
RESSGN    DS   1              ;SIGN OF RESULT OF FLOATING POINT OPERATION
FPSBYT    DS   1              ;FLOATING POINT SUB BYTE (FIFTH BYTE)
COEFPT    DS   2              ;POLYNOMIAL COEFFICIENT POINTER
LSTTXT    DS   2              ;CURRENT LINE POINTER DURING LIST
CURLIN    DS   2              ;CURRENT LINE # OF BASIC PROGRAM, $FFFF = DIRECT
DEVCFW    DS   1              ;TAB FIELD WIDTH
DEVLCF    DS   1              ;TAB ZONE
DEVPOS    DS   1              ;PRINT POSITION
DEVWID    DS   1              ;PRINT WIDTH
RSTFLG    DS   1              ;WARM START FLAG: $55=WARM, OTHER=COLD
RSTVEC    DS   2              ;WARM START VECTOR - JUMP ADDRESS FOR WARM START
TOPRAM    DS   2              ;TOP OF RAM
IKEYIM    DS   1              ;INKEY$ RAM IMAGE
ZERO      DS   2              ;DUMMY - THESE TWO BYTES ARE ALWAYS ZERO
...
GETNCH    DS   2              ;INC  <CHARAD+1      ;INCREMENT LS BYTE OF INPUT POINTER
          DS   2              ;BNE  GETCCH         ;BRANCH IF NOT ZERO (NO CARRY)
          DS   2              ;INC  <CHARAD        ;INCREMENT MS BYTE OF INPUT POINTER
GETCCH    DS   1              ;DB   $B6            ;OP CODE OF LDA EXTENDED
CHARAD    DS   2              ;THESE 2 BYTES CONTAIN ADDRESS OF THE CURRENT
                              ;CHARACTER WHICH THE BASIC INTERPRETER IS
                              ;PROCESSING
          DS   3              ;JMP  BROMHK         ;JUMP BACK INTO THE BASIC RUM

VAB       DS   1              ;LOW ORDER FOUR BYTES OF THE PRODUCT
VAC       DS   1              ;OF A FLOATING POINT MULTIPLICATION
VAD       DS   1              ;THESE BYTES ARE USE AS RANDOM DATA
VAE       DS   1              ;BY THE RND STATEMENT

; EXTENDED BASIC VARIABLES
TRCFLG    DS   1              ;TRACE FLAG 0=OFF ELSE=ON
USRADR    DS   2              ;ADDRESS OF THE START OF USR VECTORS
...
IRQVEC    DS   3
FRQVEC    DS   3
USRJMP    DS   3              ;JUMP ADDRESS FOR BASIC'S USR FUNCTION
RVSEED    DS   1              ;FLOATING POINT RANDOM NUMBER SEED EXPONENT
          DS   4              ;MANTISSA: INITIALLY SET TO $804FC75259

;USR FUNCTION VECTOR ADDRESSES (EX BASIC ONLY)
USR0      DS   2              ;USR 0 VECTOR
          DS   2              ;USR 1
          DS   2              ;USR 2
          DS   2              ;USR 3
          DS   2              ;USR 4
...
STRSTK    DS   8*5            ;STRING DESCRIPTOR STACK
LINHDR    DS   2              ;LINE INPUT BUFFER HEADER
LINBUF    DS   LBUFMX+1       ;BASIC LINE INPUT BUFFER
STRBUF    DS   41             ;STRING BUFFER

PROGST    DS   1              ;START OF PROGRAM SPACE
;         INTERRUPT VECTORS
          ORG  $FFF2
SWI3      DS   2
SWI2      DS   2
...
LA10D     DB   16             ;TAB FIELD WIDTH
          DB   64             ;LAST TAB ZONE
          DB   80             ;PRINTER WIDTH
          DB   0              ;LINE PRINTER POSITION
          DW   LB44A          ;ARGUMENT OF EXEC COMMAND - SET TO ‘FC’ ERROR
; LINE INPUT ROUTINE
          INC  CHARAD+1
          BNE  LA123
          INC  CHARAD
...
; THESE BYTES ARE MOVED TO ADDRESSES $A7-$B1
          ...
          DB   $80            ;RANDOM SEED
          DW   $4FC7          ;RANDON SEED OF MANTISSA
          DW   $5259          ;.811635157
          
; BASIC COMMAND INTERPRETATION TABLE ROM IMAGE
COMVEC    DB   50             ;50 BASIC COMMANDS
          DW   LAA66          ;POINTS TO RESERVED WORDS
          DW   LAB67          ;POINTS TO JUMP TABLE FOR COMMANDS
          DB   29             ;29 BASIC SECONDARY COMMANDS
          DW   LAB1A          ;POINTS TO SECONDARY FUNCTION RESERVED WORDS
          DW   LAA29          ;POINTS TO SECONDARY FUNCTION JUMP TABLE
          DW   0              ;NO MORE TABLES (RES WORDS=0)
          DW   0              ;NO MORE TABLES
          DW   0              ;NO MORE TABLES
          DW   0              ;NO MORE TABLES
          DW   0              ;NO MORE TABLES
          DW   0              ;NO MORE TABLES (SECONDARY FNS =0)

; COPYRIGHT MESSAGES
LA147     DB   "6809 EXTENDED BASIC"
          DB   CR
          DB   "(C) 1982 BY MICROSOFT"
LA156     DB   CR,CR
LA165     DB   $00
...
TOK_INPUT EQU  $0089
          ...
          DW   RENUM          ;9D
          DW   EDIT           ;9E
TOK_HIGH_EXEC EQU  $009E

; ERROR MESSAGES AND THEIR NUMBERS AS USED INTERNALLY
LABAF     DB   "NF"           ;0 NEXT WITHOUT FOR
          DB   "SN"           ;1 SYNTAX ERROR
          DB   "RG"           ;2 RETURN WITHOUT GOSUB
          DB   "OD"           ;3 OUT OF DATA
          DB   "FC"           ;4 ILLEGAL FUNCTION CALL
          DB   "OV"           ;5 OVERFLOW
          DB   "OM"           ;6 OUT OF MEMORY
          DB   "UL"           ;7 UNDEFINED LINE NUMBER
          DB   "BS"           ;8 BAD SUBSCRIPT
          DB   "DD"           ;9 REDIMENSIONED ARRAY
          DB   "/0"           ;10 DIVISION BY ZERO
          DB   "ID"           ;11 ILLEGAL DIRECT STATEMENT
          DB   "TM"           ;12 TYPE MISMATCH
          DB   "OS"           ;13 OUT OF STRING SPACE
          DB   "LS"           ;14 STRING TOO LONG
          DB   "ST"           ;15 STRING FORMULA TOO COMPLEX
          DB   "CN"           ;16 CAN'T CONTINUE
          DB   "FD"           ;17 BAD FILE DATA
          DB   "AO"           ;18 FILE ALREADY OPEN
          DB   "DN"           ;19 DEVICE NUMBER ERROR
          DB   "IO"           ;20 I/O ERROR
          DB   "FM"           ;21 BAD FILE MODE
          DB   "NO"           ;22 FILE NOT OPEN
          DB   "IE"           ;23 INPUT PAST END OF FILE
          DB   "DS"           ;24 DIRECT STATEMENT IN FILE
; ADDITIONAL ERROR MESSAGES ADDED BY EXTENDED BASIC
L890B     DB   "UF"           ;25 UNDEFINED FUNCTION (FN) CALL
L890D     DB   "NE"           ;26 FILE NOT FOUND
...
LAEEA     DB   SKP1LD         ;SKPILD SKIP ONE BYTE; LDA #$5F
...
READ      LDX  DATPTR         ;GET ‘READ’ START ADDRESS
          DB   SKP1LD         ;SKIP ONE BYTE - LDA #*$4F
...
LB0E8     DB   "?EXTRA IGNORED" ;?EXTRA IGNORED MESSAGE
...
LB145     DB   $7D            ;OP CODE OF TST $1A01 - SKIP TWO BYTES (DO
...
LB1CB     DB   $64            ;RELATIONAL COMPARISON FLAG
LB1CC     DW   LB2F4          ;JUMP ADDRESS
...
LB290     JSR  GETNCH         ;GET AN INPUT CHARACTER (SECONDARY TOKEN)
          ...
          CMPB #(NUM_SEC_FNS-1)*2 ;SECONDARY FUNCTIONS - 1
...
; DO SECONDARIES (LEFT$, RIGHT$, MID$)
          ...
          DB   $8E            ;OP CODE OF LDX# - SKlP 2 BYTES
...
LB2D4     DB   SKP1LD         ;SKIP ONE BYTE - ‘OR’ FLAG = $4F
...
LB3DF     DB  $90,$80,$00,$00,$00 ;FLOATING POINT -32768
...
LBAC4     RTS
LBAC5     DB   $81,$00,$00,$00,$00 ;FLOATING POINT CONSTANT 1.0
...
LBB7C     RTS
LBB7D     DB   $84,$20,$00,$00,$00 ;FLOATING POINT CONSTANT 10
; DIVIDE FPA0 BY 10
...
LBDB6     DB   $9B,$3E,$BC,$1F,$FD ; 99999999.9
LBDBB     DB   $9E,$6E,$6B,$27,$FD ; 999999999
LBDC0     DB   $9E,$6E,$6B,$28,$00 ; 1E + 09
...
LBEBC     LDX  #STRBUF+3      ;RESET POINTER TO START OF BUFFER
          RTS
;
LBEC0     DB   $80,$00,$00,$00,$00 ;FLOATING POINT .5
;
;* TABLE OF UNNORMALIZED POWERS OF 10
LBEC5     DB   $FA,$0A,$1F,$00 ;-100000000
LBEC9     DB   $00,$98,$96,$80 ;10000000
LBECD     DB   $FF,$F0,$BD,$C0 ;-1000000
LBED1     DB   $00,$01,$86,$A0 ;100000
LBED5     DB   $FF,$FF,$D8,$F0 ;-10000
LBED9     DB   $00,$00,$03,$E8 ;1000
LBEDD     DB   $FF,$FF,$FF,$9C ;-100
LBEE1     DB   $00,$00,$00,$0A ;10
LBEE5     DB   $FF,$FF,$FF,$FF ;-1
...
RSEED     DW   $40E6          ;CONSTANT RANDOM NUMBER GENERATOR SEED
          DW   $4DAB
...
LBFBD     DB   $83,$49,$0F,$DA,$A2 ;6.28318531 (2*PI)
LBFC2     DB   $7F,$00,$00,$00,$00 ;.25


LBFC7     DB   6-1            ;SIX COEFFICIENTS
LBFC8     DB   $84,$E6,$1A,$2D,$1B ; -((2*PI)**11)/11!
LBFCD     DB   $86,$28,$07,$FB,$F8 ; ((2*PI)**9)/9!
LBFD2     DB   $87,$99,$68,$89,$01 ; -((2*PI)**7)/7!
LBFD7     DB   $87,$23,$35,$DF,$E1 ; ((2*PI)**5)/5!
LBFDC     DB   $86,$A5,$5D,$E7,$28 ; -((2*PI)**3)/3!
LBFE1     DB   $83,$49,$0F,$DA,$A2 ;

          DB   $A1,$54,$46,$8F,$13 ;UNUSED GARBAGE BYTES
          DB   $8F,$52,$43,$89,$CD ;UNUSED GARBAGE BYTES
; EXTENDED BASIC
...
L83AB     DB   $81,$49,$0F,$DA,$A2 ;1.57079633 (PI/2)
...
L83E0     DB   $0B            ;TWELVE COEFFICIENTS
L83E1     DB   $76,$B3,$83,$BD,$D3 ;-6.84793912E-04 1/23
L83E6     DB   $79,$1E,$F4,$A6,$F5 ;+4.85094216E-03 1/21
L83EB     DB   $7B,$83,$FC,$B0,$10 ;-0.0161117018
L83F0     DB   $7C,$0C,$1F,$67,$CA ;0.0342096381
L83F5     DB   $7C,$DE,$53,$CB,$C1 ;-0.0542791328
L83FA     DB   $7D,$14,$64,$70,$4C ;0.0724571965
L83FF     DB   $7D,$B7,$EA,$51,$7A ;-0.0898023954
L8404     DB   $7D,$63,$30,$88,$7E ;0.110932413
L8409     DB   $7E,$92,$44,$99,$3A ;-0.142839808
L840E     DB   $7E,$4C,$CC,$91,$C7 ;0.199999121
L8413     DB   $7F,$AA,$AA,$AA,$13 ;-0.333333316
L8418     DB   $81,$00,$00,$00,$00 ;1

;* TCHEBYSHEV MODIFIED TAYLOR SERIES COEFFICIENTS FOR LN(X)

L841D     DB   3              ;FOUR COEFFICIENTS
L841E     DB   $7F,$5E,$56,$CB,$79 ;0.434255942
L8423     DB   $80,$13,$9B,$0B,$64 ;0.576584541
L8428     DB   $80,$76,$38,$93,$16 ;0.961800759
L842D     DB   $82,$38,$AA,$3B,$20 ;2.88539007

L8432     DB   $80,$35,$04,$F3,$34 ;1/SQR(2)

L8437     DB   $81,$35,$04,$F3,$34 ;SQR(2)

L843C     DB   $80,$80,$00,$00,$00 ;-0.5

L8441     DB   $80,$31,$72,$17,$F8 ;LN(2)
...
L84C4     DB   $81,$38,$AA,$3B,$29 ;1.44269504 ( CF )

; TCHEBYSHEV MODIFIED TAYLOR SERIES COEFFICIENTS FOR E**X

L84C9     DB   7              ;EIGHT COEFFICIENTS
L84CA     DB   $71,$34,$58,$3E,$56 ;2.14987637E-05: 1/(7!*(CF**7))
L84CF     DB   $74,$16,$7E,$B3,$1B ;1.4352314E-04 : 1/(6!*(CF**6))
L84D4     DB   $77,$2F,$EE,$E3,$85 ;1.34226348E-03: 1/(5!*(CF**5))
L84D9     DB   $7A,$1D,$84,$1C,$2A ;9.61401701E-03: 1/(4!*(CF**4))
L84DE     DB   $7C,$63,$59,$58,$0A ;0.0555051269
L84E3     DB   $7E,$75,$FD,$E7,$C6 ;0.240226385
L84E8     DB   $80,$31,$72,$18,$10 ;0.693147186
L84ED     DB   $81,$00,$00,$00,$00 ;1
...
; TRON
TRON      DB   SKP1LD         ;SKIP ONE BYTE AND LDA #$4F
...
L8A67     DB   SKP1LD         ;SKIP ONE BYTE - LDA #$4F
...
L8BD9     DB   "UL "          ;UNKNOWN LINE NUMBER MESSAGE
          DB   0
...
L90CB     LDA  V47            ;GET BASE 10 EXPONENT OFFSET, ADD INITIAL SHIFT COUNTER
          ...
          LDA  VD9            ;ETERMINE HOW MANY FILLER ZEROES TO THE LEFT OF THE DECIMAL
          SUBA #$09           ;OINT. GET THE NUMBER OF FORMAT PLACES TO LEFT OF DECIMAL
          SUBA V47            ;OINT, SUBTRACT THE BASE 10 EXPONENT OFFSET AND THE CONSTANT 9
...
; INTERRUPT VECTORS
          ORG  $FFF0
LBFF0     DW   $0000          ;RESERVED
LBFF2     DW   SW3VEC         ;SWI3
LBFF4     DW   SW2VEC         ;SWI2
LBFF6     DW   FRQVEC         ;FIRQ
LBFF8     DW   IRQVEC         ;IRQ
LBFFA     DW   SWIVEC         ;SWI
LBFFC     DW   NMIVEC         ;NMI
LBFFE     DW   RESVEC         ;RESET
